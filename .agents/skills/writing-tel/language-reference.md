...
- `#include stdio` → `#include <stdio.h>` (the identifier becomes `<name.h>`). Use this directive — do NOT write `#include <stdio.h>` inside an inline C block.
- `#define A 3`, `#define SQUARE(x) ((x) * (x))` — copied through verbatim, but **semantic analysis does not expand them**. Referencing a `#define`d name in an expression (e.g. `i < A`) fails as an "unknown identifier". Use the literal value in TEL code; `#define` is only useful as text passed to the generated C.
