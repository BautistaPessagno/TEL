...
Exit 0 = valid syntax + semantics. Note: C code generation is currently a stub, so `run.sh` validates the program but does **not** emit C yet — don't expect C output or try to compile the result.
